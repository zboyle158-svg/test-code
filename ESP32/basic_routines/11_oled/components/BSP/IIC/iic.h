...
/*
 * IIC 控制块：保存一条 I2C 总线的运行时信息。
 * port 是 I2C_NUM_0/I2C_NUM_1；scl、sda 是实际 GPIO；
 * init_flag 保存 i2c_driver_install() 的返回值，供上层判断初始化是否成功。
 */
...
/* 一段连续的读写缓冲区，多个 i2c_buf_t 可以组成一次复合事务。 */
...
#define IIC_FREQ                        400000                                                          /* IIC通信频率：400 kHz */
...
/* 函数声明 */
i2c_obj_t iic_init(uint8_t iic_port);                                                                   /* 初始化 I2C 控制器并返回控制块 */
esp_err_t i2c_transfer(i2c_obj_t *self, uint16_t addr, size_t n, i2c_buf_t *bufs, unsigned int flags);  /* 按 flags 组织一次 I2C 事务 */
